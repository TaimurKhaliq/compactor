...
export function renderSkillMarkdown(candidate: CandidateSkill): string {
  return [
    ...
    "",
    "## Naming explanation",
    `- Generic category selected: ${candidate.genericCategory}`,
    `- Generic fallback: ${candidate.genericFallbackName}`,
    `- Domain terms used: ${candidate.domainTerms.length > 0 ? candidate.domainTerms.join(", ") : "None"}`,
    `- Terms rejected as noisy: ${candidate.rejectedNoisyTerms.length > 0 ? candidate.rejectedNoisyTerms.join(", ") : "None"}`,
    ...candidate.namingReasons.map((reason) => `- ${reason}`),
    "",
